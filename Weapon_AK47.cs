datablock AudioProfile(RPX_ak47Fire1Sound)
{
	filename    = "./wav/ak47_fire1.wav";
	description = HeavyClose3D;
	preload = true;
};

datablock AudioProfile(RPX_ak47Fire2Sound)
{
	filename    = "./wav/ak47_fire2.wav";
	description = HeavyClose3D;
	preload = true;
};

datablock AudioProfile(RPX_ak47Fire3Sound)
{
	filename    = "./wav/ak47_fire3.wav";
	description = HeavyClose3D;
	preload = true;
};

datablock AudioProfile(RPX_ak47ReloadMagOutSound)
{
	filename    = "./wav/ak47_reload_magout.wav";
	description = AudioClosest3D;
	preload = true;
};

datablock AudioProfile(RPX_ak47ReloadMagInSound)
{
	filename    = "./wav/ak47_reload_magin.wav";
	description = AudioClosest3D;
	preload = true;
};

datablock AudioProfile(RPX_ak47ReloadEndEmptySound)
{
	filename    = "./wav/ak47_reload_bolt.wav";
	description = AudioClosest3D;
	preload = true;
};

datablock DebrisData(RPX_ak47MagDebris)
{
	shapeFile = "./dts/ak47_mag.dts";
	lifetime = 2.0;
	minSpinSpeed = -700.0;
	maxSpinSpeed = -600.0;
	elasticity = 0.5;
...
datablock ItemData(RPX_ak47Item)
{
	category = "Weapon";
	className = "Weapon";

	shapeFile = "./dts/ak47_item.dts";
	rotate = false;
	mass = 1;
	density = 0.2;
	elasticity = 0.2;
...
datablock ShapeBaseImageData(RPX_ak47Image)
{
	shapeFile = "./dts/ak47_image.dts";
	emap = true;
	...
	doColorShift = true;
	colorShiftColor = RPX_ak47Item.colorShiftColor;
...
function RPX_ak47Image::AEOnFire(%this,%obj,%slot)
{
	...
	%obj.stopAudio(0); 
	%obj.playAudio(0, RPX_ak47Fire @ getRandom(1, 3) @ Sound);	

	Parent::AEOnFire(%this, %obj, %slot);
}

function RPX_ak47Image::onReloadStart(%this, %obj, %slot)
{
	%obj.stopAudio(2);
	%obj.playAudio(2, RPX_ak47ReloadMagOutSound);
}

function RPX_ak47Image::onReload(%this, %obj, %slot)
{	
	%obj.stopAudio(1);
	%obj.playAudio(1, RPX_ak47ReloadMagInSound);
...
function RPX_ak47Image::onReload2Start(%this, %obj, %slot)
{
	%obj.stopAudio(2);
	%obj.playAudio(2, RPX_ak47ReloadMagOutSound);
}

function RPX_ak47Image::onReload2(%this, %obj, %slot)
{
	%obj.stopAudio(1);
	%obj.playAudio(1, RPX_ak47ReloadMagInSound);

	%obj.reload2Schedule = %obj.schedule(950, stopAudio, 2);
	%obj.reload3Schedule = %obj.schedule(950, playAudio, 2, RPX_ak47ReloadEndEmptySound);
	
	%this.onMagDrop(%obj,%slot);
	%obj.reload4Schedule = schedule(getRandom(300,400),0,serverPlay3D,AEMagMetalAR @ getRandom(1,3) @ Sound,%obj.getPosition());
}

function RPX_ak47Image::onDryFire(%this, %obj, %slot)
{
	%obj.aeplayThread(2, plant);
	serverPlay3D(AEDryFireSound, %obj.getHackPosition());
}

function RPX_ak47Image::onReady(%this,%obj,%slot)
{
...
function RPX_ak47Image::onMount(%this,%obj,%slot)
{
	%this.AEMountSetup(%obj, %slot);
	parent::onMount(%this,%obj,%slot);
}

function RPX_ak47Image::onUnMount(%this, %obj, %slot)
{	
...
function RPX_ak47Image::onMagDrop(%this,%obj,%slot)
{
	%a = new Camera()
	{
		datablock = Observer;
	...
	%a.mountImage(RPX_ak47MagImage,0);
	%a.schedule(2500,delete);
}

datablock ShapeBaseImageData(RPX_ak47MagImage)
{
	shapeFile = "base/data/shapes/empty.dts";
	mountPoint = 0;
	offset = "-1.28 0.28 0.17";
	rotation = eulerToMatrix( "0 83 0" );
	
	casing = RPX_ak47MagDebris;
...
function RPX_ak47MagImage::onDone(%this,%obj,%slot)
{
